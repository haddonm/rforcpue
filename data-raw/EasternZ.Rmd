...
The intent of this outline manuscript is to begin to piece together analysis of the validity of using cpue with wild caught abalone. This is a synthesis of discussions about this subject that you and I started a couple of times but never really took anywhere. Its importance lies in the fact that all Australian abalone harvest strategies are empirical and use catch-rates of some kind (surveys are formalized catch-rates).

# Introduction

Australia still has a number of wild abalone fisheries but, as with all abalone fisheries, their management remains difficult for many reasons, the most important of which is the complex spatial structure exhibited by the biological properties at different locations. Along with other processes, this spatial structure influences the productivity of different reefs and the size structure of the abalone typically caught in different locations.

The patchiness that is typical in abalone fisheries also influences observed catch-rates, and all Australian abalone fisheries use catch-rates of one kind or another in their harvest strategies, even where the harvest strategy is informal. The Tasmanian fishery remains one of the largest in the world in terms of yield, and this present work aims to examine the validity of using catch-rates in an important part of that fishery. 

Despite their on-going use, abalone catch-rates (cpue) have a bad reputation and are considered unreliable because they are generally considered to suffer from hyper-stability, with a risk of serial depletion of patches of abalone along a coast. The idea is that divers move from patch to patch so as to maintain cpue (hence hyper-stable) but in the process they deplete each patch leading to the eventual collapse of the fishery. The mis-match between the reputation of abalone cpue and their continued use in practice means that a closer examination of significant current abalone fisheries may illuminate under what circumstances cpue can be informative and when they should not be trusted.

An examination of whether abalone cpue can be used as a valid foundation of processes to provide management advice entails examining how well the current conceptual framework used to describe cpue matches the reality of the operation of abalone fisheries. The primary assumption when using cpue from commercial fisheries is that they provide a valid measure of the relative abundance of exploitable biomass. As a relative measure they are only potentially useful given a time-series of estimates.

Another major assumption behind the notion that cpue provides a measure of relative abundance is that the members of stock constitute a dynamic-pool of individuals. The dynamic-pool assumption is fundamentally about the mixing rate within a population and implies that fishing mortality (or other processes) applied in one part of a fishery will influence all other parts of the fishery within a short period of time. With Tasmanian blacklip abalone (_Haliotis rubra_) the mixing rate is very local and very low (Miller _et al_, 2009) and seriously breaks the dynamic pool assumption that is a part of so much of fisheries science theory. This affects the potential relationship between observed catch-rates and relative abundance in terms of what is termed the 'catchability'. Catchability is usually defined as the proportion of the exploitable biomass taken by one unit of effort.

$$I_t=qB^{\lambda}_t$$
where $I_t$ is the cpue during time $t$, $B^{\lambda}_t$ is the average exploitable biomass available during time $t$ and $\lambda$ is a coefficient that determines the linearity (or otherwise) of the relationship between biomass and cpue; typically it is assumed to equal 1.0, implying a linear relationship. Finaly,$q$ is the catchability for a given fishing method (this could also be given a time subscript). 

With the complex spatial structure exhibited by abalone populations a problem arises with the usual notion of rapidly mixing exploitable biomass across a fishery. With highly mixed scale-fisheries, estimates of $q$ are invariably very small numbers because the biomass of the entire stock is effectively being exposed to the fishing mortality (a dynamic pool). However, an abalone diver operating on a particular reef could plausibly take an appreciable proportion of all legal sized abalone (exploitable biomass) in a single dive, so $q$ could easily be of the order of 0.5 or higher. Movement of abalone is mostly through larval dispersal rather than individuals migrating, so the impaact of a fishing event is mostly countered by recruitment into legal sizes from under-sized animals growing above the legal size. Generally the individual dive events, that make up the fishery, operate at small scales, such as individual reefs, whereas the assessment and management tends to operate at larger scales (e.g. the east coast of Tasmania). Fundamentally, the assumption of an abalone fishery forming a dynamic pool fails, which means the classical equation relating cpue to biomass could be misleading.

Despite all that, abalone cpue trends along the Tasmanian east coast appear to be remarkably consistent. This is unexpected exactly because we consider the assumption of the dynamic pool to be incorrect. It would be helpful to determine whether this coincidence of cpue trends reflects some pattern in how the dvers operate along the coast or for some other driver. We will thus illustrate the coast-wide cpue and determine how much it differs by statistical block. These analyses will include an examination of which divers are doing the fishing in different places and other aspects of the fleet dynamics that might influence the eventual cpue. Is there a within season depletion that is detectable? 
...
### We will need to examine and illustrate

* the standardized cpue trend for each statistical block.

* which divers fished in which blocks?

* how do divers distribute their effort through the fishing year?

To do this we will first need to select the data to be used to focus on the fishery and avoid noise, although as a sensiivity each standardization should be repeated without data selection (other than Eastern zone and years)
...
* remove very small catch rates and very large catch rates, but define what those are.
...
### Data Characterization prior to Selection
...
ab3 <- makecategorical(labelM,ab2) # use all divers
mods <- makemodels(labelM)
model <- standLM(mods,ab3,splabel,console=TRUE) 
pick <- which (ab2$count > 1)
ab4 <- makecategorical(labelM,ab2[pick,]) # only use divers >1 year
mods <- makemodels(labelM)
model1 <- standLM(mods,ab4,splabel,console=TRUE) 
```

The optimum model is essentially coincident when all divers are used or only those who have fished for more than 1 year, with the year parameters varying only at the third, fourth or fifth decimal place. This is not surprising given that removing those 33 divers only removed 218 records out of 83668 records, and only 51.56t out of 22698.53t (0.227% of total catches from the selected data).
...
Harley, S., Myers, R.A., and A. Dunn (2001) Is catch-per-unit-effort proportional to abundance? _Canadian Journal of Fisheries and Aquatic Sciences_ __58__:1760–1772.
...
99.957 percent of all records (84101/84137) reported 12 hours or less of effort, while 99.915 percent of all records reported >0 kg and 1000kg or less for their catch (even though one records reported 3200kg). In addition, there were 120 records reporting 0.0 hours of effort, and 55 records (out of 84137) reported 0.0 catch. When estimating cpue these too need to be removed.


```{r fig.width=6, fig.height=4.0}
pick <- which(ab$zone == "E")
ab1 <- droplevels(ab[pick,])
totC <- sum(ab1$catch,na.rm=TRUE)/1000
records <- countgtzero(ab1$catch)
totrec <- nrow(ab1)
parset(plots=c(2,2),margin=c(0.3,0.35,0.15,0.1))
oute <- hist(ab1$hours,breaks=30,col=2,main="hours",xlab="")
outc <- hist(ab1$catch,breaks=30,col=2,main="catch",xlab="")
pickE <- which(ab1$hours <= 12)
oute2 <- hist(ab1$hours[pickE],breaks=24,col=2,main="hours",xlab="")
pickC <- which((ab1$catch <= 1000) & (ab1$catch > 0))
outc2 <- hist(ab1$catch[pickC],breaks=26,col=2,main="catch",xlab="")
```
These plots relate to the whole of the blacklip fishery in Tasmania. The left-hand column relates to effort while the right-hand column to catch. Note the contraction of the x-axis from the top row to the bottom reflecting the selected upper limit on effort and catch

```{r}
  efftab <- cbind(oute$breaks[1:30],oute$counts,round(oute$counts/sum(oute$counts),7),
                  cumsum(oute$counts/sum(oute$counts)),cumsum(oute$counts))
  colnames(efftab) <- c("breaks","count","PropCount","CumulProp","CumulCount")
  efftab
```

```{r}
  cattab <- cbind(outc$breaks[1:32],outc$counts,round(outc$counts/sum(outc$counts),7),
                  cumsum(outc$counts/sum(outc$counts)),cumsum(outc$counts))
  colnames(cattab) <- c("breaks","count","PropCount","CumulProp","CumulCount")
  cattab
``` 
...
# first select eastern zone and limits on effort and catch
...
outce3 <- hist(ab1$LnCE,breaks=30,col=2,main="log(cpue)",xlab="")
ncurv3 <- addnorm(outce3,ab1$LnCE)
lines(ncurv3$x,ncurv3$y,lwd=2,col=4)
outce4 <- hist(ab1$LnCE[pickce],breaks=30,col=2,main="log(cpue)",xlab="",xlim=c(1.5,6))
ncurv <- addnorm(outce4,ab1$LnCE[pickce])
lines(ncurv$x,ncurv$y,lwd=2,col=4)
```


Which suggests that we should be removing any cpue values > 340 kg/hr and < 5 kg/hr, a loss of 160 or 0.2% of records and 8.31t or 0.036% of the catch. 



```{r}
sumcum <- cumsum(outce3$counts)
cpue <- cbind(outce3$counts,sumcum,sumcum/tail(sumcum,1),exp(outce3$mids))
rownames(cpue) <- outce3$mids
colnames(cpue) <- c("count","cumsum","proportion","cpue")
round(cpue,5)
```
